...
describe('Testing Client class', function () {
  ...
  describe('Client() constructor', function () {
    ...
    it('returns a valid GSF object with no headers', function (done) {
      ...
      expect(clientNoHead.address).to.equal(
        config.localHTTPServerNoHeader.address
      );
  ...
  describe('.services()', function () {
    it('retrieves the services', function () {
      ...
      return client.services().then((serviceList) => {
  ...
  describe('.jobs()', function () {
    ...
    it('retrieves a list of jobs', function () {
      ...
      return client.jobs().then((jobList) => {
        ...
        .then((jobList1) => {
          client
            ...
            .then((jobList2) => {
              ...
              expect(parseInt(jobList2[0].jobId, 10)).to.equal(
                parseInt(jobList1[0].jobId, 10) + offset
              );
              done();
            })
            .catch((err) => {
        ...
        .then((jobList1) => {
          client
            ...
            .then((jobList2) => {
              ...
              expect(parseInt(jobList1[0].jobId, 10)).to.be.below(
                parseInt(jobList2[0].jobId, 10)
              );
              done();
            })
            .catch((err) => {
    ...
    it('retrieves a list of jobs with limit=10', function () {
      ...
      return client.jobs(filter).then((jobList) => {
        expect(jobList).to.be.an('array');
        expect(jobList.length).to.be.above(1);
    ...
    it('rejects promise if error from request', function () {
      ...
      return assert.isRejected(badclient.jobs(), /Error requesting jobs/);
    });
  });
  ...
  describe('.jobInfoList()', function () {
    ...
    it('retrieves a list of jobs', function () {
      this.timeout(config.testTimeout2);
      return client.jobInfoList().then((jobList) => {
        const { jobs } = jobList;
    ...
    it('retrieves a list of jobs with offset=1', function () {
      this.timeout(config.testTimeout1);
      const offset = 1;
      return client.jobInfoList().then((jobList1) => {
        const { jobs: jobs1 } = jobList1;
        return client.jobInfoList({ offset: offset }).then((jobList2) => {
          const { jobs: jobs2 } = jobList2;
          expect(jobs2).to.be.an('array');
          expect(parseInt(jobs2[0].jobId, 10)).to.equal(
            parseInt(jobs1[0].jobId, 10) + offset
          );
        });
      });
    });
    ...
    it('retrieves a list of jobs with limit=10', function () {
      ...
      return client.jobInfoList(filter).then((jobList) => {
        const { jobs } = jobList;
        expect(jobs).to.be.an('array');
        expect(jobs.length).to.be.above(1);
    ...
    it('retrieves a list of jobs with totals for each status', function () {
      ...
      return client.jobInfoList(searchOpts).then((jobList) => {
        expect(jobList.total).to.be.a('number');
      ...
      it('fires when job completes', function () {
        ...
        setTimeout(() => {
          return client
            ...
            .then((results) => {
              assert(
                completedListener.calledOnceWith({
                  jobId: jobId,
                  success: true
                })
              );
        ...
        setTimeout(() => {
          return client
            ...
            .then((results) => {
              ...
              assert(
                succeededListener.calledOnceWith({
                  jobId: jobId,
                  success: true
                })
              );
        ...
        setTimeout(() => {
          return client
            ...
            .catch((err) => {
              ...
              assert(completedListener.calledOnce);
              assert(
                failedListener.calledOnceWith({ jobId: jobId, success: false })
              );
            });
        }, config.submitTimeout);
      });
      ...
      it('fires when job emits progress', function () {
        ...
        setTimeout(() => {
          return client
            ...
            .then((results) => {
              expect(progressListener.callCount).to.equal(
                testData.sleepTask.parameters.N_PROGRESS
              );
              assert(completedListener.calledOnce);
              const args = progressListener.args.map((arg) => arg[0]);
              const progress = args.map((arg) => arg.progress);

              args.should.all.have.property(
                'message',
                testData.sleepTask.parameters.PROGRESS_MESSAGE
              );
